...
import "./Post1.css"; // Using the same CSS file for consistency
import physicalHealthQuestions from "../physicalHealthQuestions";
...
const Post2 = () => {
  ...
  const question = physicalHealthQuestions[currentQuestion];
  ...
  const handleNextClick = () => {
    ...
    while (nextIndex < physicalHealthQuestions.length) {
      if (shouldShowQuestion(physicalHealthQuestions[nextIndex])) {
  ...
  const handleGetResults = async () => {
    ...
    try {
      const formattedAnswers = Object.entries(answers).map(([id, answer]) => {
        const q = physicalHealthQuestions.find(q => q.id === id);
        return {
          question_id: id,
      ...
      const response = await fetch('http://localhost:8000/analyze', {
        ...
        body: JSON.stringify({
          quiz_id: 2, // Changed to 2 for Physical Health
      ...
      setAdvice(data);
      await saveToHistory("Physical Health 🏋️‍♂️🩺", data.recommendations);
    ...
    <div className="quiz-page">
      <div className="card-container">
        <div className="header">
          <h1>Physical Health Quiz 🏋️‍♂️🩺</h1>
          {!isFinished ? (
        ...
        {isFinished && advice && (
          <div className="results-container" style={{ textAlign: 'left', marginTop: '20px', animation: 'fadeIn 0.5s ease-in' }}>
            <h3 style={{ color: '#2c4a1f', marginBottom: '15px'}}>Your Physical Health Report</h3>
            ...
            <h4 style={{ color: '#558d2a', marginBottom: '10px'}}>Health Recommendations:</h4>
        ...
        {isFinished && loading && (
          <div style={{ padding: '20px', color: '#558d2a' }}>✨ Analyzing your health data...</div>
        )}
        ...
        {!isFinished ? (
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
            {!advice && !loading && (
              <button className="next-btn" onClick={handleGetResults}>✨ Get AI Advice</button>
            )}
