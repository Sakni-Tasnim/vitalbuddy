...
import "./Post1.css"; 
import nutritionQuestions from "../nutritionQuestions";
...
const Post5 = () => {
  ...
  const question = nutritionQuestions[currentQuestion];
  ...
  const handleNextClick = () => {
    ...
    while (nextIndex < nutritionQuestions.length) {
      if (shouldShowQuestion(nutritionQuestions[nextIndex])) {
  ...
  const handleGetResults = async () => {
    ...
    try {
      const formattedAnswers = Object.entries(answers).map(([id, answer]) => {
        const q = nutritionQuestions.find(q => q.id === id);
        return {
          question_id: id,
      ...
      await saveToHistory("Nutrition & Fuel🍎🥗", data.recommendations);
      ...
      <div className="card-container">
        ...
        {isFinished && advice && (
          <div className="results-container" style={{ textAlign: 'left', marginTop: '20px', animation: 'fadeIn 0.5s ease-in' }}>
            <h3 style={{ color: '#2c4a1f', marginBottom: '15px'}}>Nutritional Profile</h3>
            ...
            <h4 style={{ color: '#558d2a', marginBottom: '10px'}}>Dietary Adjustments:</h4>
        ...
        {isFinished && loading && (
          <div style={{ padding: '20px', color: '#558d2a' }}>✨ Analyzing your fuel intake...</div>
        )}
        ...
        {!isFinished ? (
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
            {!advice && !loading && (
              <button className="next-btn" onClick={handleGetResults}>✨ Get My Report</button>
            )}
