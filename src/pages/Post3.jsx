...
import "./Post1.css"; 
import sleepQuestions from "../sleepQuestions";
...
const Post3 = () => {
  ...
  const handleNextClick = () => {
    ...
    while (nextIndex < sleepQuestions.length) {
      if (shouldShowQuestion(sleepQuestions[nextIndex])) {
  ...
  const handleGetResults = async () => {
    ...
    try {
      const formattedAnswers = Object.entries(answers).map(([id, answer]) => {
        const q = sleepQuestions.find(q => q.id === id);
        return {
          question_id: id,
      ...
      const response = await fetch('http://localhost:8000/analyze', {
        ...
        body: JSON.stringify({
          quiz_id: 3, // ID for Sleep & Recovery
      ...
      await saveToHistory("Sleep & Recovery 🌙🛌", data.recommendations);
      ...
      <div className="card-container">
        ...
        {isFinished && advice && (
          <div className="results-container" style={{ textAlign: 'left', marginTop: '20px', animation: 'fadeIn 0.5s ease-in' }}>
            <h3 style={{ color: '#2c4a1f', marginBottom: '15px'}}>Recovery Insights</h3>
            ...
            <h4 style={{ color: '#558d2a', marginBottom: '10px'}}>Optimal Sleep Tips:</h4>
        ...
        {isFinished && loading && (
          <div style={{ padding: '20px', color: '#558d2a' }}>✨ Calculating recovery score...</div>
        )}
        ...
        {!isFinished ? (
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
            {!advice && !loading && (
              <button className="next-btn" onClick={handleGetResults}>✨ View Insights</button>
            )}
